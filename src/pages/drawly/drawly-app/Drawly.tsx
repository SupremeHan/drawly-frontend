...
export function Drawly() {
  ...
  const onDragEnd = (evt: Konva.KonvaEventObject<DragEvent>) => {
    // console.log(evt.target.className );

    // const stage = stageRef.current?.getPointerPosition();
    // const { x, y } = stage!;
    // console.log(currentShapeId.current);
    const { x, y } = evt.target.getPosition();
    console.log(x, y);
    switch (
      getActionFromClassName(evt.target.className as ClassnameShapeType)
    ) {
      case ActionType.Arrow:
        setArrows((prevVal) =>
          prevVal.map((arrow) => {
      ...
      case ActionType.Circle:
        ...
        break;
      default:
        console.error('nevalja');
    }
  };

  type ClassnameShapeType = 'Circle' | 'Rect' | 'Arrow';

  const getActionFromClassName = (className: ClassnameShapeType) => {
    switch (className) {
      case 'Arrow':
      ...
      case 'Rect':
        return ActionType.Reactangle;

      default:
        throw new Error(`${className} is not a type`);
    }
  };
